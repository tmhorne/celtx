...
static CFStringRef GetPrintStringFromIndex(int index, MyCustomContext context);
...
extern void MyReleaseCustomContext (MyCustomContext context)
{
    ...
    MyCFAssign(NULL, context->settings.mCustomHeaderCenter);
...
static void InitSettings(MySettings* settings)
{
    ...
    settings->mCustomHeaderCenter           = MyCFAssign(NULL, settings->mCustomHeaderCenter);
}
...
static void InternSettings(CFDictionaryRef srcDict, MySettings* settings)
{
    ...
    if ((dictValue = CFDictionaryGetValue(srcDict, kPDEKeyHeaderCenter)) &&
        ...
        if (CFStringGetLength(dictValue) > 0 &&
            ! CFEqual(dictValue, CFSTR("&T")) &&
            ! CFEqual(dictValue, CFSTR("&U")) &&
            ! CFEqual(dictValue, CFSTR("&D")) &&
            ! CFEqual(dictValue, CFSTR("&P")) &&
            ! CFEqual(dictValue, CFSTR("&PT")))
                settings->mCustomHeaderCenter = MyCFAssign(dictValue, settings->mCustomHeaderCenter);
    }
...
static void SyncPaneFromSettings(MyCustomContext context)
{
    ...
    if (context->controls.headerCenterPopup) {
      if (context->settings.mCustomHeaderCenter) {
        UInt16 numChildren;
        OSErr err = CountSubControls(context->controls.headerCenterPopup,
          &numChildren);
        if (err == noErr) {
          ControlRef customMenuItem;
          err = GetIndexedSubControl(context->controls.headerCenterPopup,
            numChildren, &customMenuItem);
          if (err == noErr)
            EnableControl(customMenuItem);
        }
      }
...
static void SyncSettingsFromPane(MyCustomContext context)
{
    ...
    if (context->controls.headerLeftPopup) {
      ...
      context->settings.mHeaderLeft = MyCFAssign(GetPrintStringFromIndex(controlVal, context),
                                                 context->settings.mHeaderLeft);      
    }
    if (context->controls.headerCenterPopup) {
      controlVal = GetControl32BitValue(context->controls.headerCenterPopup);
      context->settings.mHeaderCenter = MyCFAssign(GetPrintStringFromIndex(controlVal, context),
                                                   context->settings.mHeaderCenter);
    }
    if (context->controls.headerRightPopup) {
      controlVal = GetControl32BitValue(context->controls.headerRightPopup);
      context->settings.mHeaderRight = MyCFAssign(GetPrintStringFromIndex(controlVal, context),
                                                  context->settings.mHeaderRight);
    }
    if (context->controls.footerLeftPopup) {
      controlVal = GetControl32BitValue(context->controls.footerLeftPopup);
      context->settings.mFooterLeft = MyCFAssign(GetPrintStringFromIndex(controlVal, context),
                                      context->settings.mFooterLeft);
    }
    if (context->controls.footerCenterPopup) {
      controlVal = GetControl32BitValue(context->controls.footerCenterPopup);
      context->settings.mFooterCenter = MyCFAssign(GetPrintStringFromIndex(controlVal, context),
                                                   context->settings.mFooterCenter);
    }
    if (context->controls.footerRightPopup) {
      controlVal = GetControl32BitValue(context->controls.footerRightPopup);
      context->settings.mFooterRight = MyCFAssign(GetPrintStringFromIndex(controlVal, context),
...
static int  GetIndexForPrintString(CFStringRef stringCode)
{
    if (stringCode)
    {
        ...
        if (CFEqual(stringCode, CFSTR("&PT")))
            return 6;
        // Custom
        if (CFStringGetLength(stringCode) > 0)
            return 7;
    }
...
CFStringRef GetPrintStringFromIndex(int index, MyCustomContext context)
{
    switch (index)
    {
        case 2: // title
        ...
        case 7: // Custom
            if (context->settings.mCustomHeaderCenter)
                return context->settings.mCustomHeaderCenter;
            else
                return CFSTR("");
