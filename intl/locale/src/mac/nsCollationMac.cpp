...
{
  CFIndex aLen1 = (CFIndex) string1.Length();
  CFIndex aLen2 = (CFIndex) string2.Length();
  if (aLen1 == 0) {
    *result = (aLen2 == 0) ? 0 : -1;
    return NS_OK;
  }
  ...
  const nsPromiseFlatString& flat1 = PromiseFlatString(string1);
  const nsPromiseFlatString& flat2 = PromiseFlatString(string2);
  CFStringRef aString1 = ::CFStringCreateWithCharactersNoCopy(NULL,
    (UniChar *) flat1.get(), aLen1, kCFAllocatorNull);
  CFStringRef aString2 = ::CFStringCreateWithCharactersNoCopy(NULL,
    (UniChar *) flat2.get(), aLen2, kCFAllocatorNull);
  CFOptionFlags compareFlags = 0;
  if (strength & kCollationCaseInsensitiveAscii)
    compareFlags |= kCFCompareCaseInsensitive;
  // The typo in "Insenstive" reflects nsICollation.idl
  if (strength & kCollationAccentInsenstive)
    compareFlags |= kCFCompareNonliteral;
  *result = (PRInt32) ::CFStringCompare(aString1, aString2,
