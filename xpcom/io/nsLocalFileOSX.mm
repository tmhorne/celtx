...
NS_IMETHODIMP
nsLocalFile::GetExtensionHidden(PRBool *_retval)
{
  // Check we are correctly initialized.
  CHECK_mBaseRef();
  ...
  OSStatus status = ::LSCopyItemInfoForURL(mBaseRef,
    kLSRequestExtensionFlagsOnly, &infoRecord);
  if (status < 0)
    return NS_ERROR_FAILURE;
...
NS_IMETHODIMP
nsLocalFile::SetExtensionHidden(PRBool aHidden)
{
  // Check we are correctly initialized.
  CHECK_mBaseRef();

  ::LSSetExtensionHiddenForURL(mBaseRef, aHidden);
