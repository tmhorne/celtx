...
// CXPopupDelegate forwards the popup action to the nsFilePicker
@interface CXPopupDelegate : NSObject {
  nsFilePicker* picker;
}
@property(nonatomic,readwrite) nsFilePicker* picker;
- (void)popupSelected:(id)sender;
@end
@implementation CXPopupDelegate
@synthesize picker;
- (void)popupSelected:(id)sender {
  if (picker)
    picker->PopupSelected((NSPopUpButton *)sender);
}
@end
...
nsFilePicker::~nsFilePicker()
{
  [mPopupFilterArray release];
  [mPopupDelegate release];
}
...
NSView* nsFilePicker::GetAccessoryView()
{
  ...
  mPopupDelegate = [[CXPopupDelegate alloc] init];
  mPopupDelegate.picker = this;
  [popupButton setAction:@selector(popupSelected:)];
  [popupButton setTarget:mPopupDelegate];
...
nsFilePicker::PutLocalFile(const nsString& inTitle, const nsString& inDefaultName, nsILocalFile** outFile)
{
  ...
  *outFile = nsnull;
  ...
  mAllowOther = NO;
  mPopupFilterArray = [GeneratePopupFilterArray(&mAllowOther) retain];
  ...
  if (mPopupFilterArray && mSelectedTypeIndex >= 0 &&
      (NSUInteger)(mSelectedTypeIndex) < [mPopupFilterArray count]) {
    NSDictionary* entry = [mPopupFilterArray objectAtIndex:mSelectedTypeIndex];
    [thePanel setAllowedFileTypes:[entry objectForKey:@"extensions"]];
    [thePanel setAllowsOtherFileTypes:mAllowOther];
  }
  else
    [thePanel setAllowedFileTypes:nil];
...
void
nsFilePicker::PopupSelected(NSPopUpButton* popup)
{
  if (mMode != modeSave)
    return;

  mSelectedTypeIndex = [popup indexOfSelectedItem];
  if (mPopupFilterArray && mSelectedTypeIndex >= 0 &&
      (NSUInteger)(mSelectedTypeIndex) < [mPopupFilterArray count]) {
    NSDictionary* entry = [mPopupFilterArray objectAtIndex:mSelectedTypeIndex];
    [mPanel setAllowedFileTypes:[entry objectForKey:@"extensions"]];
    [mPanel setAllowsOtherFileTypes:mAllowOther];
  }
  else
    [mPanel setAllowedFileTypes:nil];
}
...
// Like GenerateFilterList, but in a format more practical for the
// save panel's accessory view. The array consists of dictionaries
// of this form:
// { title: NSString, extensions: NSArray [ NSString ] }
NSArray*
nsFilePicker::GeneratePopupFilterArray (PRBool* outAllowsAny)
{
  NS_OBJC_BEGIN_TRY_ABORT_BLOCK_NIL;
  ...
  if (outAllowsAny)
    *outAllowsAny = PR_FALSE;

  NSCharacterSet* stripCharSet =
    [NSCharacterSet characterSetWithCharactersInString:@". *"];

  NSMutableArray* filterArray = [NSMutableArray array];

  for (PRInt32 i = 0; i < mFilters.Count(); ++i) {
    nsString* filterWide = mFilters[i];

    if (filterWide->Equals(NS_LITERAL_STRING("*"))) {
      if (outAllowsAny)
        *outAllowsAny = PR_TRUE;
      continue;
    }

    NSString* filterString = [NSString stringWithCharacters:filterWide->get()
      length:filterWide->Length()];
    NSRange stripRange = [filterString rangeOfCharacterFromSet:stripCharSet];
    while (stripRange.length != 0) {
      // stringByReplacingCharactersInRange:withString: is not supported
      // prior to Mac OS X v10.5
      NSMutableString* tempString = [NSMutableString stringWithString:filterString];
      [tempString replaceCharactersInRange:stripRange withString:@""];
      filterString = [[tempString copy] autorelease];
      stripRange = [filterString rangeOfCharacterFromSet:stripCharSet];
    }

    nsString* titleWide = mTitles[i];
    if (! titleWide || titleWide->IsEmpty())
      titleWide = filterWide;
    NSString* titleString = [NSString stringWithCharacters:titleWide->get()
      length:titleWide->Length()];

    [filterArray addObject:[NSDictionary dictionaryWithObjectsAndKeys:
      titleString, @"title",
      [filterString componentsSeparatedByString:@";"], @"extensions",
      nil]];
  }
