...
/*
 * This allows individual windows to disable the About or Preferences
 * menu items on OS X, since there is a single instance of the application
 * menu that gets swapped into the active menu bar, so it is not recreated
 * for each window.
 *
 * Note that the [NSMenuItem setEnabled:] method doesn't do anything for
 * the About or Prefereces items because they are validated by their parent
 * menu. Instead, their targets are set to nil to cause them to be
 * automatically disabled by the NSMenuValidation protocol.
 */
void nsMenuBarX::UpdateApplicationMenu()
{
  NSMenu* appMenu = [[[[NSApp mainMenu] itemAtIndex:0] submenu] retain];

  if (sAboutItemContent) {
    PRBool disabled = PR_TRUE;
    if (mAboutItemContent)
      disabled = mAboutItemContent->AttrValueIs(kNameSpaceID_None,
        nsWidgetAtoms::disabled, nsWidgetAtoms::_true, eCaseMatters);
    [[appMenu itemWithTag:eCommand_ID_About]
      setTarget:(disabled ? nil : nsMenuBarX::sNativeEventTarget)];
  }

  if (sPrefItemContent) {
    PRBool disabled = PR_TRUE;
    if (mPrefItemContent)
      disabled = mPrefItemContent->AttrValueIs(kNameSpaceID_None,
        nsWidgetAtoms::disabled, nsWidgetAtoms::_true, eCaseMatters);
    [[appMenu itemWithTag:eCommand_ID_Prefs]
      setTarget:(disabled ? nil : nsMenuBarX::sNativeEventTarget)];
  }
