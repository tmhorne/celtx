...
var gController = new FormController('generic');
...
function ready () {
  ...
  try {
    ...
    var model = new RDFModel(schemads);
    var type = gController.model.target(gController.res, PROP('rdf:type'));
    var typename = model.target(type, PROP('rdfs:label'));
    sheet.contentDocument.title = typename.value;
    var typeSpan = sheet.contentDocument.getElementById("formTypeSpan");
    typeSpan.replaceChild(
      sheet.contentDocument.createTextNode(typename.value + ": "),
      typeSpan.firstChild);
