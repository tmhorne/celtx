...
!define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\Greyfirst\${BrandFullNameInternal}\${AppVersion} (${AB_CD})\Main"
...
Section "-InstallStartCleanup"
  ...
  ${CleanUpdatesDir} "Greyfirst\Celtx"
...
Section "-Application" APP_IDX
  ...
  WriteRegStr HKLM "$0\celtx" "" "URL:Celtx Protocol"
  WriteRegStr HKLM "$0\celtx" "URL Protocol" ""
  WriteRegStr HKLM "$0\celtx\DefaultIcon" "" "$INSTDIR\${FileMainEXE},0"
  WriteRegStr HKLM "$0\celtx\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""

  WriteRegStr HKLM "$0\celtxs" "" "URL:Secure Celtx Protocol"
  WriteRegStr HKLM "$0\celtxs" "URL Protocol" ""
  WriteRegStr HKLM "$0\celtxs\DefaultIcon" "" "$INSTDIR\${FileMainEXE},0"
  WriteRegStr HKLM "$0\celtxs\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""

  WriteRegStr HKLM "$0\.celtx" "" "CeltxProject"
  WriteRegStr HKLM "$0\.tceltx" "" "CeltxTemplate"

  WriteRegStr HKLM "$0\CeltxProject" "" "Celtx Document"
  WriteRegStr HKLM "$0\CeltxProject\DefaultIcon" "" "$INSTDIR\${FileMainEXE},1"
  WriteRegStr HKLM "$0\CeltxProject\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""

  WriteRegStr HKLM "$0\CeltxTemplate" "" "Celtx Template"
  WriteRegStr HKLM "$0\CeltxTemplate\DefaultIcon" "" "$INSTDIR\${FileMainEXE},1"
  WriteRegStr HKLM "$0\CeltxTemplate\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""
  ...
  ${If} $TmpVal == "HKLM"
    ...
    StrCpy $0 "Software\Greyfirst\Celtx\InstallInfo"
...
!ifndef NO_INSTDIR_FROM_REG
    SetShellVarContext all      ; Set SHCTX to HKLM
    ${GetSingleInstallPath} "Software\Greyfirst\${BrandFullNameInternal}" $R9
