...
Section "Uninstall"
  ...
  ${If} "$0" == "1"
    ${un.DeleteRelativeProfiles} "Greyfirst\Celtx"
    RmDir "$APPDATA\Greyfirst\Extensions\celtx@celtx.com"
    RmDir "$APPDATA\Greyfirst\Extensions"
    RmDir "$APPDATA\Greyfirst"
  ${EndIf}

  SetShellVarContext current  ; Set SHCTX to HKCU
  ${un.RegCleanMain} "Software\Greyfirst"
  ...
  ${un.RegCleanAppHandler} "CeltxProject"
  ${un.RegCleanAppHandler} "CeltxTemplate"

  ClearErrors
  ReadRegStr $R9 HKCR "CeltxProject" ""
  ; Don't clean up the file handlers if the CeltxProject key still exists since
  ; there should be a second installation that may be the default file handler
  ${If} ${Errors}
    ${un.RegCleanFileHandler}  ".celtx"  "CeltxProject"
    ${un.RegCleanFileHandler}  ".tceltx" "CeltxTemplate"
  ${EndIf}

  SetShellVarContext all  ; Set SHCTX to HKLM
  ${un.GetSecondInstallPath} "Software\Greyfirst" $R9
  ${If} $R9 == "false"
    SetShellVarContext current  ; Set SHCTX to HKCU
    ${un.GetSecondInstallPath} "Software\Greyfirst" $R9
  ${EndIf}
  ...
  ; installing even if there is another install of Celtx that is set as the
  ...
  ${If} $R9 == "false"
    ...
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
    StrCpy $0 "Software\Classes\MIME\Database\Content Type\application/x-xpinstall;app=celtx"
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
  ${Else}
    ReadRegStr $R1 HKLM "$0" ""
  ...
  ${un.CleanUpdatesDir} "Greyfirst\Celtx"
  ...
  ; If celtx.exe was successfully deleted yet we still need to restart to
  ; remove other files create a dummy celtx.exe.moz-delete to prevent the
