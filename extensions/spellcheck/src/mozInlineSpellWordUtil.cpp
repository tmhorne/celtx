...
FindNextTextNode(nsIDOMNode* aNode, PRInt32 aOffset, nsIDOMNode* aRoot)
{
  ...
  if (child) {
    ...
    if (child) {
      // tony: Wha? If it's beyond the end of the child list, and you check the
      // last child, it will stop early if the last child has children! To start
      // at the next correct node, we want to find the last *descendant*.
      nsCOMPtr<nsIDOMNode> subchild(child);
      while (subchild) {
        child = subchild;
        child->GetLastChild(getter_AddRefs(subchild));
      }
...
// Pre-declaration for FindPrevNode
 ...
 *
 * <body><h1>Foo</h1>bar</body>
 *
 * This will see the word "Foobar" when the cursor is in bar, unless we
 * stop searching when we descend backwards into the h1 element.
 */
static nsIDOMNode*
...
mozInlineSpellWordUtil::BuildSoftText()
{
  ...
  while (node) {
    ...
    // Added extra arguments for our shortcut code
    CheckLeavingBreakElementClosure closure = { mCSSView, PR_FALSE };
    nsIDOMNode* tmp = node;
    node = FindPrevNode(node, mRootNode, CheckLeavingBreakElement, &closure);
    if (closure.mLeftBreakElement) {
      node = tmp;
      break;
    }
  }
