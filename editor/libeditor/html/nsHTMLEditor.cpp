...
{
#if 1
  nsCOMPtr<nsIDOMNode> blockParent = GetBlockNodeParent(aNode);
  if (blockParent && nsHTMLEditUtils::IsParagraph(blockParent)) {
    PRBool hasOmit = PR_FALSE;
    nsCOMPtr<nsIDOMElement> parentElement(do_QueryInterface(blockParent));
    if (parentElement)
      parentElement->HasAttribute(NS_LITERAL_STRING("omit"), &hasOmit);
    if (hasOmit)
      return PR_FALSE;
  }
#endif
