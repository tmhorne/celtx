...
NS_IMETHODIMP nsPlaintextEditor::GetWordCount(PRUint32* _retval) {
  NS_ENSURE_ARG(_retval);
  nsIDOMElement* rootElement = GetRoot();
  if (! rootElement)
    return NS_ERROR_FAILURE;

  *_retval = GetWordCountForNode(rootElement);

  return NS_OK;
}

NS_IMETHODIMP
nsPlaintextEditor::GetSelectionWordCount(nsISelection* aSelection,
                                         PRUint32* _retval) {
  NS_ENSURE_ARG(aSelection);
  NS_ENSURE_ARG(_retval);

  PRUnichar* str;
  nsresult rv = aSelection->ToString(&str);
  if (NS_FAILED(rv))
    return rv;
  if (! str)
    return NS_ERROR_FAILURE;
  ...
  PRUint32 total = 0;
  NS_NAMED_LITERAL_STRING(delims, " \t\r\n");
  PRBool inWord = PR_FALSE;
  PRUnichar* cursor = str;
  while (*cursor) {
    if (delims.FindChar(*cursor++) >= 0) {
        inWord = PR_FALSE;
    }
    else if (! inWord) {
      ++total;
...
PRUint32 nsPlaintextEditor::GetWordCountForNode(nsIDOMNode* aNode) {
  if (! aNode)
    return 0;

  PRUint16 nodeType;
  nsresult rv = aNode->GetNodeType(&nodeType);
  if (NS_FAILED(rv))
    return 0;
  ...
  if (nodeType == nsIDOMNode::TEXT_NODE) {
    ...
    NS_NAMED_LITERAL_STRING(delims, " \t\r\n");
    nsAutoString value;
    rv = aNode->GetNodeValue(value);
    if (NS_FAILED(rv))
      return 0;

    // Count the words in the text node
    nsAutoString::const_iterator start, end;
    value.BeginReading(start);
    value.EndReading(end);
    PRBool inWord = PR_FALSE;
    while (start != end) {
      if (delims.FindChar(*start++) >= 0) {
        inWord = PR_FALSE;
      }
      else if (! inWord) {
        ++total;
  ...
  rv = aNode->HasChildNodes(&hasChildren);
  ...
  PRUint32 length;
  rv = children->GetLength(&length);
  if (NS_FAILED(rv))
    return 0;

  for (PRUint32 i = 0; i < length; ++i) {
    nsIDOMNode* item;
    rv = children->Item(i, &item);
    if (NS_FAILED(rv) || ! item)
      return 0;

    total += GetWordCountForNode(item);
  }
  return total;
}
