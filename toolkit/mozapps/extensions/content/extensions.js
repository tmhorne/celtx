...
function openBrowser (url) {
  ...
  if (! eps.externalProtocolHandlerExists(uri.scheme)) {
    if (navigator.platform.indexOf('Mac') != -1 &&
      ...
      var xdgfile = Components.classes["@mozilla.org/file/local;1"]
        .createInstance(Components.interfaces.nsILocalFile);
      xdgfile.init("/usr/bin/xdg-open");
      var xdgfile = pathToFile("/usr/bin/xdg-open");
      if (! xdgfile.exists()) {
        xdgfile = Components.classes["@mozilla.org/file/local;1"]
          .createInstance(Components.interfaces.nsILocalFile);
        xdgfile.init("/bin/xdg-open");
      }
      if (xdgfile.exists()) {
...
function showView(aView) {
  ...
  try {
    showCheckUpdatesAll = !gPref.getBoolPref(PREF_EXTENSIONS_HIDE_UPDATES_BTN);
  } catch (e) { }
...
function rebuildPluginsDS()
{
  ...
  // If building with --disable-plugins, this will throw an exception
  try {
...
function Startup()
{
  ...
  var viewGroup = document.getElementById("viewGroup");
  var visiblePanes = 0;
  for (var i = 0; i < viewGroup.childNodes.length; ++i)
    if (! viewGroup.childNodes[i].hasAttribute("hidden")) ++visiblePanes;
  if (visiblePanes < 2)
    viewGroup.setAttribute("hidden", "true");
...
function enableRestartButton() {
  ...
  if (!notification) {
    ...
    var showbuttons = ! gPref.getBoolPref("extensions.hideRestartButton");
    ...
    addonsMsg.appendNotification(message, "restart-app",
                                 ...
                                 addonsMsg.PRIORITY_WARNING_HIGH,
