...
//----------------------------------------------------------------------------------------
{
  ...
  nsCOMPtr<nsIUTF8StringEnumerator> strenum(do_QueryInterface(catenum));
  if (! strenum)
    return errAEEventNotHandled;

  PRBool hasMore = PR_FALSE;
  nsCAutoString entry;
  while (NS_SUCCEEDED(rv = strenum->HasMore(&hasMore)) && hasMore) {
    rv = strenum->GetNext(entry);
    if (NS_FAILED(rv))
        return errAEEventNotHandled;

    nsXPIDLCString handlerID;
    rv = catman->GetCategoryEntry("command-line-handler",
      entry.get(), getter_Copies(handlerID));
    if (! handlerID)
      continue;

    nsCOMPtr<nsIMacAppHelper> handler(do_GetService(handlerID.get()));
    if (! handler)
      continue;

    rv = handler->OpenFile(inFile);
    if (NS_FAILED(rv))
      return errAEEventNotHandled;
